...
After the same verification, the operator may fetch the installed solution macro with a read-only `Macros Macro Get` command. Macro Settings can instead read `Video Input Connector` configuration and `Cameras` status to discover camera sources without fetching a macro or writing any RoomOS input configuration. Camera status is joined by CameraId first. If connector configuration omits that value, the next unclaimed status camera supplies its `id` as the backup ControlId. Only the matched `xStatus Cameras` entry determines connection state; if no entry remains, camera status is unavailable. Local uploads and fetched macros are parsed as data without executing their source.
